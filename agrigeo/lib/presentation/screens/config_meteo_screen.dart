...
class ConfigMeteoScreen extends StatefulWidget {
  const ConfigMeteoScreen({super.key});

  @override
  State<ConfigMeteoScreen> createState() => _ConfigMeteoScreenState();
}

class _ConfigMeteoScreenState extends State<ConfigMeteoScreen> {
  ...
  Future<void> _saveApiKey() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString(AppConstants.openWeatherApiKey, apiKey);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Configuration Météo'),
      ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'Pour utiliser la fonctionnalité météo, vous devez obtenir une clé API gratuite depuis openweathermap.org',
                      ),
                      const SizedBox(height: 8),
                      TextButton.icon(
                        onPressed: () {
                          // Ouvrir le site OpenWeather
                        },
